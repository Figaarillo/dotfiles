...
class UserEntity extends BaseEntity<IUserEntity> implements IUserEntity {
  ...
  constructor(props: IUserEntity) {
    super(props)
    this._firstName = this.data.firstName.value
    this._lastName = this.data.lastName.value
    this._phoneNumber = this.data.phoneNumber.value
    this._birthday = this.data.birthday.value
  }
