...
export class UserCreatorUseCase {
  ...
  constructor(userRepository: UserRepository, uuidGenerator: UuidGenerator | any) {
   ...
   * @returns user crated
   */
  async run(params: IUser): Promise<IUser> {
    const user = new User({
      id: new UserId(this._uuidGenerator.generate()),
      name: new UserName(params.name._value),
      username: new UserUsername(params.username._value),
      age: new UserAge(params.age._value),
    })
