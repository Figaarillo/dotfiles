...
class Cart {
  ...
  async addProduct(sku, quantity) {
    ...
    try {
      ...
      if (product.stock < quantity) {
        ...
        Logger.show(
          `No hay suficiente stock disponible. Se agregaron ${cantidad} unidades.`
        );
      }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (currentQuanity < quantity) {
          ...
          Logger.show(
            `No hay suficientes unidades en el carrito. Se eliminarán ${cantidad} unidades.`
          );
        }
  ...
  inreaseStockQuantity(sku, quantity) {
    const productToModify = supermarketPryiwoducts.find(
      product => product.sku === sku
    );
    productToModify.stock += quantity;
  }
