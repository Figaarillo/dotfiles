...
class Cart {
  ...
  async addProduct(sku, quantity) {
    Logger.show( `Agregando ${quantity} ${quantity < 2 ? 'unidad' : 'unidades'} de ${sku}...`);
    try {
      ...
      if (product.stock < quantity) {
        ...
        Logger.show(`No hay suficiente stock disponible. Se agregaron ${cantidad} unidades.`);
      }
  ...
  deleteProduct(sku, quantity) {
    return new Promise(async (resolve, reject) => {
      try {
        Logger.show(`Eliminando ${quantity} ${quantity < 2 ? 'unidad' : 'unidades'} de ${sku}...`); Logger.show('Buscando producto en la base de datos...')
        const product = await this.findProductBySku(sku)
        ...
        if (currentQuanity < quantity) {
          ...
          Logger.show(`No hay suficientes unidades en el carrito. Se eliminarán ${cantidad} unidades.`);
        }
  ...
  inreaseStockQuantity(sku, quantity) {
    const productToModify = supermarketProducts.find(product => product.sku === sku)
    productToModify.stock += quantity
  }
  ...
  cleanCategories() {
    const skusToRemove = this.products.filter((product) => product.quantity === 0).map((product) => product.sku);
    this.categorias = new Set([...this.categories].filter((category) => !skusToRemove.includes(category)));
  }
...
|                           |
| Categorías:${[...this.categorias].map((categoria) => `\n\t* ${categoria}`)}
|                           |
-----------------------------
