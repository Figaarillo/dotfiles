...
total: \t\t\t$${this.precioTotal}
  ...
  #checkAndRemoveDepletedProducts() {
    const depletedProducts = this.productos.filter(p => p.cantidad === 0);

    const depletedSkus = depletedProducts.map(product => product.sku);

    this.#cleanCategories(depletedSkus);

    return this.productos.filter(product => product.cantidad !== 0);
  }

  #cleanCategories(skusToClean) {
    const categoriesToClean = skusToClean.map(
      product => productosDelSuper.find(p => p.sku === product).categoria
    );

    console.log(categoriesToClean);
    const test = ['higiene', 'limpieza']
    console.log(this.categorias)

    this.categorias = this.categorias.filter(
      category =>!test.includes(category)
    );
