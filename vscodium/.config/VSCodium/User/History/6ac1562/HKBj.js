...
class Carrito {
  ...
  async agregarProducto(sku, cantidad) {
    Logger.show(`Agregando ${cantidad} ${cantidad < 2 ? 'unidad' : 'unidades'} de ${sku}...`)
    ...
    try {
      ...
      if (producto.stock < cantidad) {
        ...
        Logger.show(`No existen suficientes productos en el stock del supermercado, por lo tanto solo podra llevar ${cantidad} de unidades`)
      }

      // Saca los productos del stock del supermercado
      this.#modifyStockQuantity(sku, cantidad)
  ...
  eliminarProducto(sku, quantity) {
    return new Promise((resolve, reject)  => {
      Logger.show(`Eliminando ${quantity} ${quantity < 2 ? 'unidad' : 'unidades'} de ${sku}...`)

      Logger.show('Buscando producto en la base de datos...')
      findProductBySku(sku).then((product) => {
        Logger.show('Producto encontrado')
        console.log({ product })

        // TODO: si el producto existe se llama al metodo de modificar quantity pasandole un numero o se la modifica para que ahora remueva elementos
        if (product.stock < quantity) {
          quantity = product.stock
          Logger.show(`No existen suficientes productos en el stock del supermercado, por lo tanto solo podra llevar ${quantity} de unidades`)
        }
        ...
        // this.productos = this.#cleanProductsFromTheCart()
...
total: \t\t\t$${this.precioTotal}
  ...
  #modifyStockQuantity(sku, quantity) {
    const productToModify = productosDelSuper.find(product => product.sku === sku)
    productToModify.stock -= quantity
    productosDelSuper.push(productToModify)
  }
  
  #cleanProductsFromTheCart() {
    return this.productos.filter(product => product.cantidad !== 0)
  }

  #cleanCategories() {
    const abaibleSku = this.productos.map(product => product.sku)

    // const categoryToSku = productosDelSuper.filter(product => {
    //   if (product.sku ) {
    //
    //   }
    // })
    
    const categories = abaibleSku.map(({sku}) => (productosDelSuper.find(p => p.sku === sku)).categoria)
  }
}
