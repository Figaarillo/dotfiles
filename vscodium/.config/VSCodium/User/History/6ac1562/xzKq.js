...
total: \t\t\t$${this.precioTotal}
  ...
  #checkAndRemoveDepletedProducts() {
    ...
    depletedProducts.map(product => product.sku);
  ...
  #cleanCategories(skus) {
    const availableSku = this.productos.reduce((acc, curr) => {
      if (skus.find(sku => sku === curr.sku)) {
        console.log(skus.find(sku => sku === curr.sku));
        return acc.push(curr.sku)
      }
    }, []);
