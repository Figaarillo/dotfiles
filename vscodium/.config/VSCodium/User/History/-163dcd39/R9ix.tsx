...
class UserService {
  ...
  createUser(nameStr: string, emailStr: string) {
...
function App() {
  ...
  const user = service.createUser('John Doe', 'johndoe@example.com'); const repository = new InMemoryUserRepository();
  const [service] = useState(new UserService(repository));
  ...
  if (user) {
    ...
    return (
      <div>
        ...
        <div>{JSON.stringify(retrievedUser, null, 2)}</div>
      </div>
    );
  } else {
