...
import { UserAge, UserId, UserName, UserUsername } from 'user/domain/entities/user/value-objects'
import { Uuid } from 'user/domain/value-objects/uuid';
import { v4 as uuid } from 'uuid';
 (async () => {
    const inMemoryUserRepository = new InMemoryUserRepository()

    // Creating user
    const userCreatorUseCase = new UserCreatorUseCase(inMemoryUserRepository, new Uuid(uuid()))
